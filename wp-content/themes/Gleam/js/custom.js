(function($){
	$(document).ready(function(){
		...
		// handle internal links click
		$( 'body' ).on( 'click', 'a[href*="#"]:not([href="#"])', function( event ) {
			var this_href = $( this ).attr( 'href' );

			// scroll to the correct position and prevent default event
			et_scroll_to_element( this_href );
		...
		$.address.crawlable(true).change( function(event) {
			...
			// do not proceed if internal scroll is in process.
			if ( 'yes' === $( 'body' ).data( 'et_internal_scroll' ) ) {
				$( 'body' ).data( 'et_internal_scroll', 'no' );
				return;
			}

			// update scroll_to_element var if URL contains the internal #href
			if ( -1 !== event.value.indexOf( '#' ) ) {
				var url_split = event.value.split( '#' );

				scroll_to_element = '#' + url_split[1];
			}

			// add the previous_href data to body to update the address properly in et_scroll_to_element function
			$( 'body' ).data( 'previous_href', event.value );
				...
				$('#temp_content').html('').load( et_site_data.site_url+event.value+' #wrapper', function(){
					...
					if ( is_homepage ){
						$main_menu.slideUp( menu_slide_speed, function(){
							et_ajax_load_new_page('menu', scroll_to_element);
						} );
					} else {
						et_ajax_load_new_page('no_menu', scroll_to_element);
					}
		...
		// Perform scrolling to internal element on page
		function et_scroll_to_element( element_href, update_address ) {
			var $place = $( element_href ),
				current_address = $( 'body' ).data( 'previous_href' ),
				new_address = -1 === current_address.indexOf( element_href ) ? ( current_address + element_href ) : current_address,
				need_address_update = typeof update_address === 'undefined' ? 'yes' : update_address,
				scroll_position = '';
			...
			scroll_position = $place.offset().top - 100;

			$et_main_content.data('jsp').scrollTo( 0, scroll_position );

			// update adderess if needed
			if ( 'yes' === need_address_update ) {
				// update et_internal_scroll value to tell the other functions that internal scroll is performed
				$( 'body' ).data( 'et_internal_scroll', 'yes' );
				$.address.value( new_address );
			}
		}

		function et_ajax_load_new_page( menu, scroll_to ){
			...
			if ( !footer_menu_click ){
				...
				$et_footer_menu.css({'display':'block','opacity':0, bottom: '-48px'}).animate( { opacity : 1, bottom : '0' }, animation_speed, function(){
					...
					// scroll to element on page if needed
					if ( '' !== scroll_to ) {
						et_scroll_to_element( scroll_to, 'no' );
					}
				} );
			} else {
